...
export default function OutputPage() {
  ...
  // Load existing paper if already generated
  useEffect(() => {
    setPaper(null);
    ...
    getGeneratedPaper(assignmentId)
      ...
      .catch(() => {
        // Paper not ready yet, WS will update
      });
  }, [assignmentId]);

  // Update paper when WS delivers result
  useEffect(() => {
    if (job.result && job.assignmentId === assignmentId) {
      setPaper(job.result);
    }
  ...
  return (
    <div className="h-screen overflow-hidden bg-[#faf8f4] flex">
      {/* Desktop Sidebar — fixed, full height, does not scroll */}
      <div
      ...
      <div className="flex-1 flex flex-col min-w-0 h-screen overflow-hidden">
        ...
        <header className="lg:hidden flex items-center justify-between px-4 py-4 bg-white border-b border-[#e8e4dc]">
          <div className="flex items-center gap-3">
            ...
            <div className="flex items-center gap-2">
              <div className="w-7 h-7 bg-[#e24a2c] rounded-lg flex items-center justify-center">
                <span className="font-display font-black text-white text-xs">V</span>
              </div>
              <span className="font-display font-bold text-[#1a1713] text-sm">VedaAI</span>
            </div>
          </div>
          <div className="flex items-center gap-3">
        ...
        {/* ── STICKY BOTTOM ACTION BAR — always visible, never scrolls ── */}
        {paper && (
